...
export class RelatoreDetailComponent implements OnInit {
    ...
    constructor(
        ...
        this.route.params.subscribe(async params => {
            let currentRelatore;
            if (params?.id) {
                const data: any = await this.relatoriService.getRelatore(params.id);
    ...
    save() {
        ...
        bsModalRef.content.eventYes.subscribe(async res => {
            ...
            if (this.currentRelatore) {
                res1 = await this.relatoriService.updateRelatore(this.form.value);
            } else {
                const data = {...this.form.value};
                delete data.id;
                res1 = await this.relatoriService.insertRelatore(data);
            }
